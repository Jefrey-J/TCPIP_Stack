...
static void _pkt_receive(node_t *receving_node, char *pkt_with_aux_data, unsigned int pkt_size) {
    ...
    if(!recv_intf) {
        printf("Error : packet received on unknown interface %s on node %s\n", recv_intf->if_name, receving_node->node_name);
        return; 
    }
    ...
    while(1) {
        ...
        ITERATE_GLTHREAD_BEGIN(&topo->node_list, curr) {
            ...
            if(FD_ISSET(node->udp_sock_fd, &active_sock_fd_set)) {
                ...
                bytes_recvd = recvfrom(node->udp_sock_fd, (char *)recv_buffer, MAX_PACKET_BUFFER_SIZE, 0, (struct socketaddr *)&sender_addr, &addr_len);
